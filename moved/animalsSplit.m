% Prepare animals dataset (with corresponding word vectors). Ouputs files
% to image_data/animals/ and word_data/<word_dataset_name>/animals/.

function [] = animalsSplit(word_dataset_name)

basedir = 'image_data/images/animals/';
s = load([basedir 'data.mat']);
load([basedir 'meta_all.mat']);

load(['word_data/' word_dataset_name '/vocab.mat']);
load(['word_data/' word_dataset_name '/embeddings.mat']);
...
% Save word table for animals.
word_outputdir = ['word_data/' word_dataset_name '/animals'];
