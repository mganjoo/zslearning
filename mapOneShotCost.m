function [cost, grad] = mapOneShotCost( theta, data, params )
...
cost = 0.5 * (1 / numImages * (sum(sum((w(:,1:end-1) - a2(:,1:end-1)).^2)) + params.oneShotMult*sum((w(:,end) - a2(:,end)).^2))) + params.autoencMult * (0.5 / numAllImages * (sum(sum((hAll-allImgs).^2))) + sparsity) + reg;
